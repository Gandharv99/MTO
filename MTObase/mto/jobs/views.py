...
from .models import Microtasks,MAL_Requirements,MtoJobCategories
...
def handleSubmit(request):
    if request.method == 'POST':
        ...
        job = Microtasks.objects.get(id=Name_of_the_micro_task)
        cat = MtoJobCategories.objects.get(id=Category_of_the_Microtask)
        data = MAL_Requirements(MAL_Job_Identification_Number=MAL_Job_Identification_Number, Assembly_line_ID=Assembly_line_ID,
